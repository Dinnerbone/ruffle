...
fn caps_to_cap_style<'gc, B: Backend>(
    activation: &mut Activation<'_, 'gc, '_, B>,
    caps: Value<'gc, B>,
...
fn joints_to_join_style<'gc, B: Backend>(
    activation: &mut Activation<'_, 'gc, '_, B>,
    joints: Value<'gc, B>,
    miter_limit: f64,
...
fn draw_round_rect<'gc, B: Backend>(
    activation: &mut Activation<'_, 'gc, '_, B>,
    this: Option<Object<'gc, B>>,
    args: &[Value<'gc, B>],
) -> Result<Value<'gc, B>, Error> {
